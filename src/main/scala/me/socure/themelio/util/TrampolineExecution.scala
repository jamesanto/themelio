package me.socure.themelio.util
    ...
    def execute(runnable: Runnable): Unit = {
      local.get match {
        ...
        case next: Runnable =>
          ...
          val runnables = new JArrayDeque[Runnable](4)
          runnables.addLast(next)
          runnables.addLast(runnable)
          local.set(runnables)
        case arrayDeque: JArrayDeque[_] =>
          // Add this Runnable to the end of the existing ArrayDeque
          val runnables = arrayDeque.asInstanceOf[JArrayDeque[Runnable]]
          runnables.addLast(runnable)
        case illegal =>
